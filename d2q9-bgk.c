...
int timestep(const t_param params, t_speed* cells, t_speed* tmp_cells, int* obstacles, Array noObs)
{
  ...
  collision(params, cells, tmp_cells, Array noObs);
  return EXIT_SUCCESS;
}
...
int collision(const t_param params, t_speed* cells, t_speed* tmp_cells, Array noObs)
{
  ...
  ** are in the scratch-space grid */
  for (int jj = 0; jj < noObs->used; jj++)
  {
    ...
    float u_x = (tmp_cells[i].speeds[1]
                  + tmp_cells[i].speeds[5]
                  + tmp_cells[i].speeds[8]
                  - (tmp_cells[i].speeds[3]
                     + tmp_cells[i].speeds[6]
                     + tmp_cells[i].speeds[7]))
                 / local_density;
    /* compute y velocity component */
    float u_y = (tmp_cells[i].speeds[2]
                  + tmp_cells[i].speeds[5]
                  + tmp_cells[i].speeds[6]
                  - (tmp_cells[i].speeds[4]
                     + tmp_cells[i].speeds[7]
                     + tmp_cells[i].speeds[8]))
                 / local_density;
    ...
    for (int kk = 0; kk < NSPEEDS; kk++)
    {
      cells[i].speeds[kk] = tmp_cells[i].speeds[kk]
                                              + params.omega
                                              * (d_equ[kk] - tmp_cells[i].speeds[kk]);
    }
  }
